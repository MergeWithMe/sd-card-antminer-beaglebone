...
dd if=/dev/zero of=$NAND_IMAGE bs=1 count=0 seek=$NAND_TOTAL_SIZE
...
PARTITIONS=(
    ...
    "bootenv 0x240000 0x20000 bootenv.bin"
    ...
    "config 0x1c00000 0x1400000 config.bin"
...
mkimage -A arm -O linux -T ramdisk -C gzip -d initramfs.cpio.gz initramfs.bin.SD

echo "Creating /fpgabit partition"
# Define size (0x5000000 = 80MB)
SIZE=$((0x5000000))
# Create an empty file of the correct size
dd if=/dev/zero of=./out/fpga.ext4 bs=1 count=0 seek=$SIZE
# Format it as ext4
mkfs.ext4 ./out/fpga.ext4
...
for entry in "${PARTITIONS[@]}"; do
    ...
    dd if=$FILENAME bs=1 count=$SIZE seek=$OFFSET of=$NAND_IMAGE conv=notrunc
done
